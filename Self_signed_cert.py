...
# Author        :- ISHIMWE FABRICE ifabrice900@gmail.com
#
...
def create_CA(root_ca_path, key_path):
    ...
    ca_key.generate_key(crypto.TYPE_RSA, 4096)
                                                          ...
                                                          ca_cert = crypto.X509()
    ca_cert.set_version(2)                                ca_cert.set_serial_number(random.randint(50000000, 100000000))                                          
...
def load_CA(root_ca_path, key_path):                      ''' Load CA and Key'''
...
def create_cert(ca_cert, ca_subj, ca_key, client_cn):
    ...
    with open(client_cn + ".crt", "wt") as f:                 f.write(crypto.dump_certificate(crypto.FILETYPE_PEM, client_cert).decode("utf-8"))


    with open(client_cn + ".key", "wt") as f:                 f.write(crypto.dump_privatekey(crypto.FILETYPE_PEM, client_key).decode("utf-8"))                    
def client_varification():                                pass
